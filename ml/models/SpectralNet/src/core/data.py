...
def load_data(params):
    ...
    elif params['dset'] == 'reisse':
        x_train, x_test, y_train, y_test = get_reisse()
...
def get_reisse():
    dataset = tf.data.experimental.CsvDataset('../../../../../backup/embeddings-manual-export-data.csv', [tf.float32] * 512, header=True);
    sess = tf.Session()
    x_train =
    x_test =
    y_train =
