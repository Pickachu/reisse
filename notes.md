...
# - Predict content absorpion (perhaps with routine? perhaps anticipation?)
#
#  ➜ Figure out what i must do to use activity type as a predictor
...
#      ➜ Create daytime-activity species classifier (use tensorflow)
#        - Define activity specie mapping
#      - Test routine as a diferentiating factor
#        - Estimate routine value
#        - Decide how to overcome limitation of estimator, remember that i can't generate timeslices from 'open' occurrences yet (see far below)
#      - Audit activity type (routine) classification of habitual sleep
#        - the problem persists? createProbabilityMap receives a timestamp but activityType receives a timeslice
#
#  - Figure out why youtube videos are outperforming asana tasks all the time
#    - It still is because asana tasks have anticipation associated with them?
...
# TODO Fix chance predictor: By adding context to its learning as the trigger layer of behavior fogg model?
...
>= Current conclusion:
  ...
  - To implement content absorpion predictor I must decide if it will be responsibility area (1) or activity type (2) that will influence anticipation of content absorption.
    (1) to use responsibility area (anticipation) i need to map rescue time imported video documents to some responsibility area. I need to use data from my google activity or youtube api.
    (2) to use activity type (routine) i need to create the routine frequency estimator or improve classification of unknown activities (create species)
...
## TODO Fix youtube videos always outperforming habitual sleep
#    - Evidences
...
#     - ✖ Chance Trigger (see below)
#     - ✖ Routine: Activity Type (see below)
#     - ✖ Routine: Frequency (see below)
#     - ✖ Contextual Sensation: Sleepiness (see below)
#     - ✔ Chance High Sensation Trigger (see below)
#       - check why youtube video have higher sensation than sleep at midnight
#     - ✖ Location
#
#  Tested hypothesis
#  ✔ `Use chance sensation triggering to diferentiate habitual sleep and youtube videos`
#   - Sleep predictor was using a low sleep records amount, temporarely increased to all.
...
#    - To improve activity type prediction I need to change it's input to occurrence specie instead of activity type, because i can't generate timeslices from 'open' occurrences yet. Also when species are ready i will be able to guess occurrence start time, duration and end time based on specie.
...
#  ✖ `Use contextual sleepiness sensation to diferentiate habitual sleep and youtube videos` (it is more likely for me to sleep given that i have a high sleepiness sensation)
#    - It worked for some trainings of the chance predictor, since chance is almost aways 99% even high differences on motivation does not changes prediction.
